...
function stampPageFooters(doc, leftText) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.font(FONT).fontSize(6.5).fillColor(PALETTE.slate);
    doc.text(fitText(doc, leftText, contentWidth(doc) - 90), PAGE_MARGIN, y, { width: contentWidth(doc) - 90, lineBreak: false });
    doc.text(`Page ${i - range.start + 1} of ${total}`, PAGE_MARGIN, y, { width: contentWidth(doc), align: "right", lineBreak: false });
  }
}
