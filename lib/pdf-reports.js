...
function absentDatesFor(row, dates, limit) {
  ...
  for (let i = 0; i < dates.length; i++) {
    if (String(marks[i] || "A").toUpperCase() !== "P") out.push(shortDateOf(dates[i]));
  }
...
function drawMonthGrid(doc, y, block, rows, allDates) {
  ...
  const fixedCols = { sno: 24, name: 112, roll: 50, att: 30, held: 30, pct: 32 };
  const fixedSum = fixedCols.sno + fixedCols.name + fixedCols.roll + fixedCols.att + fixedCols.held + fixedCols.pct;
  const available = contentWidth(doc) - fixedSum;
  const dayWidth = Math.max(11, Math.min(20, available / Math.max(1, dates.length)));
  const nameWidth = Math.max(90, Math.min(300, fixedCols.name + Math.max(0, available - dayWidth * dates.length)));
  ...
  columns.push({ label: "%", width: fixedCols.pct, align: "center" });

  const rowH = 13;
  const pctIndex = columns.length - 1;
  // Us mahine ka kaunsa din "class hui" tha (kisi bhi student ka P = held).
  const heldFlags = dates.map((d, i) => rows.some((r) => String((r.dayMarks || [])[startIndex + i] || "A").toUpperCase() === "P"));
  ...
  rows.forEach((row, index) => {
    ...
    let attended = 0;
    dates.forEach((d, i) => {
      const mark = String((row.dayMarks || [])[startIndex + i] || "A").toUpperCase() === "P" ? "P" : "A";
      if (mark === "P") {
        attended++;
        presentCount[i]++;
      }
      cells.push(mark);
      colors[3 + i] = mark === "P" ? PALETTE.green : PALETTE.red;
    });
    const held = heldFlags.filter(Boolean).length;
    const pct = held ? round1((attended / held) * 100) : 0;
    cells.push(attended, held, `${pct}%`);
    colors[pctIndex] = pctColor(pct);
...
function drawOverallSummary(doc, y, meta, rows, allDates) {
  ...
  const widths = { sno: 30, name: 128, roll: 56, att: 52, held: 46, pct: 40, absent: 44, absentDates: 178 };
  const fixedSum = widths.sno + widths.name + widths.roll + widths.att + widths.held + widths.pct + widths.absent + widths.absentDates;
  ...
  rows.forEach((row, index) => {
    ...
    const absentCount = dates.length ? dates.length - (num(row.totalPresent) || 0) : 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (!list.length) {
        ...
        y = drawSectionTitle(doc, y, "Day-by-day register (P = present, A = absent)", `${dates.length} day(s) · month strip upar, DD columns neeche`);
        ...
        blocks.forEach((block, index) => {
          ...
          y = drawSectionTitle(doc, y, `${block.label} — day-by-day`, `${block.dates.length} day(s) · P = present, A = absent`);
          y = drawMonthGrid(doc, y, block, list, dates);
        });
        doc.addPage();
        y = PAGE_MARGIN;
        y = drawSectionTitle(doc, y, "Summary — poori window", "attended / held / % + kis-kis din absent (DD-MM)");
